...
Вы можете [посмотреть демо сцены из игры Portal созданной с помощью CSS онлайн][1]
или [взять исходники на Github][2]

## Установка порталов. Видео №1.

Как пример, который иллюстрирует различные стадии создания, написания стилей и
анимации для 3D сцены, эта статья воссоздаст сцены из [Обучающего видео игры Portal][3].
А именно, стилизированную, мультяшную первую часть этого видео, где напоминающий
...
потребуют от вас свойства с префиксами. Вы можете найти полные версия CSS и SASS
[на Github][7], как и HTML.

### Предстоящие исследования

Проект разрабатывался и тестировался в основном в Chrome. В нем используется
CSS который не будет работать в Internet Explorer. Он включает некоторые
интересные приемы использования CSS 3D, которые слабо распространены,
несмотря на то, что доказали свою ценность в других областях
фронтенд-разработки.

## Начнем

Нам нужно создать сцену которая будет содержать наши 3D объекты. Для этого нам
понадобится создать HTML элемент и наделить его необходимыми свойствами,
что бы браузер понимал, что внутри 3D контекст. Начнем с HTML:

    <article class="container">...</article>

В этом случае контейнером будет тэг `article`. В HTML5, `article` представляет
независимый блок данных который можно воспроизвести в любом другом месте и он
...
Свойство `transform` состоит из серии трансформаций, которые применяются к HTML
...
[Трансформация]: img/pic3.png "Трансформация"
[Набросок, иллюстрирующий необходимые трансформации]: img/pic4.png "Набросок, иллюстрирующий необходимые трансформации"
